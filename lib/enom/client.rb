  ...
  class Client
    ...
    class << self
      ...
      def request(params = {})
        ...
        response = HTTParty.get(base_uri, :query => params, :http_proxyaddr => self.proxy_ip, :http_proxyport => self.proxy_port, :http_proxyuser => self.proxy_user, :http_proxypass => self.proxy_password)
        case response.code
        when 200
